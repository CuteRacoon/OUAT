...
public class DialogueController : MonoBehaviour
{
    ...
    private IEnumerator PlayKnot(string knotName)
    {
        ...
        //StopAllCoroutines();
        StartCoroutine(PlayTextInPanel());
    }

    private IEnumerator PlayTextInPanel()
    {
        isDialoguePlaying = true;
        ...
        while (story.canContinue || story.currentChoices.Count > 0)
        {
            ...
            if (story.canContinue)
            {
                ...
                if (string.IsNullOrWhiteSpace(line))
                    continue; // пропускаем пустые реплики
                ...
                text.text = endLine;
                //yield return StartCoroutine(TypeText(endLine));
